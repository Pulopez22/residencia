<?php
	...
	if($datos_venta->rowCount()==1){
        ...
        $pdf->MultiCell(0,5,iconv("UTF-8", "ISO-8859-1","Comprado por: ".$datos_venta['usuario_nombre']." ".$datos_venta['usuario_apellido']),0,'C',false);
        $pdf->SetFont('Arial','B',10);
        $pdf->MultiCell(0,5,iconv("UTF-8", "ISO-8859-1",strtoupper("Orden de compra: ".$datos_venta['venta_id'])),0,'C',false);
        $pdf->SetFont('Arial','',9);
        ...
        foreach($venta_detalle as $detalle){
            ...
            $pdf->Cell(22,4,iconv("UTF-8", "ISO-8859-1",MONEDA_SIMBOLO.number_format($detalle['venta_detalle_precio_compra'],MONEDA_DECIMALES,MONEDA_SEPARADOR_DECIMAL,MONEDA_SEPARADOR_MILLAR)),0,0,'C');
